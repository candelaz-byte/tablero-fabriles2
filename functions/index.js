...
async function getPool() {
  if (!pool || !pool.connected) pool = await sql.connect(sqlConfig);
  return pool;
}
...
app.get('/api/fabriles', async (req, res) => {
  try {
    ...
    const gastoRows = (await p.request().query(`
      SELECT RTRIM(Dimensionvalor) AS dim, Anomes AS mes, SUM(Importemonprincipalreal) AS gastoReal
    ...
    for (const r of molinoRows) {
      ...
      volIdx['MOLINO_CASC'][`${y}-${mo.padStart(2,'0')}`] = +r.tn;
    }
    ...
    function getVol(volKey, mes) { return (volKey && volIdx[volKey]?.[mes]) ?? null; }
    ...
    function buildTab(tabKey) {
      ...
      const totV = mesesKeys.map(m => rubros.reduce((s, [, c]) => s + (getVol(c.vol, m) || 0), 0) || null);
      const totG = mesesKeys.map(m => rubros.reduce((s, [dim]) => s + (gastoIdx[dim]?.[m] || 0), 0) || null);
    ...
    }
    {
      ...
      cosMap['__MOLINO_DEP__'] = {
        n: 'Molienda+Depósito', combined: true,
        ult:   ult   != null ? Math.abs(ult)   : null,
        trim:  trim  != null ? Math.abs(trim)  : null,
        anual: anual != null ? Math.abs(anual) : null,
    ...
    res.json({
      meses, VA: buildTab('VA'), INF: buildTab('INF'), ADM: buildTab('ADM'),
      COS, gastoTotal,
...
exports.apiFabriles = functions.https.onRequest(app);
